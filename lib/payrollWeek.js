...
async function getWeekOverride(db, employeeId, start, end) {
...
async function getWeekOverridesForRange(db, start, end) {
  return await db.prepare(`
...
async function getPayrollForWeek(db, start, end) {
  ...
  const payoutFlags = await db.prepare(`
  ...
  for (const e of employees) {
    ...
    rowMap.set(e.id, mergeOverride(base, await getWeekOverride(db, e.id, start, end), weekPaid));
  }

  for (const o of await getWeekOverridesForRange(db, start, end)) {
  ...
  const totalTips = await db.prepare(`
...
async function getEmployeeWeekPay(db, employeeId, start, end) {
  ...
  return mergeOverride(base, await getWeekOverride(db, employeeId, start, end), weekPaid);
}
